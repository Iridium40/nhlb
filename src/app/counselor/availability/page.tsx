...
const S = {
  ...
  input: {
    ...
    padding: '8px 12px', fontSize: '0.85rem', fontFamily: 'Raleway, sans-serif',
    ...
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--nhlb-cream)' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{
          ...
          {grouped.length === 0 ? (
            <p style={{ fontFamily: 'Raleway, sans-serif', fontSize: '0.85rem', color: 'var(--nhlb-muted)', fontStyle: 'italic', marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              {grouped.map(g => (
                <div key={g.dayNum} style={{
                  ...
                  <span style={{ fontFamily: 'Raleway, sans-serif', fontWeight: 700, fontSize: '0.85rem', color: 'var(--nhlb-text)', minWidth: 90 }}>
                    {g.day}
                  </span>
        ...
        <div style={{
            ...
            <div style={{ marginBottom: 20 }}>
              {blockedDates.map(bd => (
                <div key={bd.id} style={{
                  ...
                  <div>
                    <span style={{ fontFamily: 'Raleway, sans-serif', fontWeight: 700, fontSize: '0.85rem', color: '#B91C1C' }}>
                      {format(new Date(bd.blocked_date + 'T12:00:00'), 'EEE, MMM d, yyyy')}
                    </span>
                    {bd.start_time && bd.end_time && (
                      <span style={{ fontFamily: 'Raleway, sans-serif', fontSize: '0.8rem', color: '#B91C1C', marginLeft: 8 }}>
                        {bd.start_time.slice(0, 5)} &ndash; {bd.end_time.slice(0, 5)}
                      </span>
                    )}
                    {!bd.start_time && (
                      <span style={{ fontFamily: 'Raleway, sans-serif', fontSize: '0.75rem', color: '#991B1B', marginLeft: 8 }}>
                        All day
                      </span>
                    )}
                    {bd.reason && (
                      <span style={{ fontFamily: 'Raleway, sans-serif', fontSize: '0.8rem', color: '#B91C1C', marginLeft: 8, fontStyle: 'italic' }}>
                        &mdash; {bd.reason}
                      </span>
                    )}
                  </div>
                  <button onClick={() => removeBlockedDate(bd.id)} style={{
                    background: 'none', border: 'none', color: '#DC2626',
                    cursor: 'pointer', fontFamily: 'Raleway, sans-serif', fontSize: '0.8rem', fontWeight: 700,
                  }}>Remove</button>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'end', flexWrap: 'wrap', marginBottom: 12 }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', paddingBottom: 4 }}>
              ...
              <span style={{ fontFamily: 'Raleway, sans-serif', fontSize: '0.8rem' }}>Full day</span>
            </label>
            {!blockFullDay && (
              <>
                <div>
