...
export async function POST(
  ...
  const { data: counselor, error: fetchErr } = await admin
    .from('counselors')
    .select('id, photo_url')
    .eq('id', counselorId)
    .single()
  ...
  const filename = `counselor-${counselorId}.${ext}`
  ...
  return NextResponse.json({ counselor: data, photo_url: publicUrlData.publicUrl })
}
...
export async function DELETE(
  ...
  if (fetchErr || !counselor) return NextResponse.json({ error: 'Counselor not found' }, { status: 404 })
  if (!counselor.photo_url) return NextResponse.json({ error: 'No photo to remove' }, { status: 400 })
