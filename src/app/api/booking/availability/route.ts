...
function getCentralOffset(date: Date): number {
  ...
  const ctStr = date.toLocaleString('en-US', { timeZone: BUSINESS_TZ })
  return (new Date(ctStr).getTime() - new Date(utcStr).getTime()) / 3600000
}

function toCentralDate(date: Date): { dateStr: string; dow: number } {
  ...
  return { dateStr: `${year}-${month}-${day}`, dow: dowMap[weekday] ?? 0 }
}

function centralSlotUTC(dateStr: string, hour: number): Date {
  const noon = new Date(`${dateStr}T12:00:00Z`)
  const offset = getCentralOffset(noon)
  const utcHour = hour - offset
  return new Date(`${dateStr}T${String(Math.floor(utcHour)).padStart(2, '0')}:00:00.000Z`)
}
...
export async function GET(req: NextRequest) {
  ...
  const nowCentral = toCentralDate(now)
  const endCentral = toCentralDate(rangeEnd)

  const { data: blocked } = await supabase
    .from('counselor_blocked_dates')
    .select('counselor_id, blocked_date, start_time, end_time')
    .in('counselor_id', ids)
    .gte('blocked_date', nowCentral.dateStr)
    .lte('blocked_date', endCentral.dateStr)

  const isBlocked = (cId: string, dateStr: string, hour: number): boolean => {
    return (blocked ?? []).some(b => {
      if (b.counselor_id !== cId || b.blocked_date !== dateStr) return false
  ...
  for (let d = 0; d <= 14; d++) {
    ...
    for (const w of windows ?? []) {
      ...
      for (let h = startHour; h < endHour; h++) {
        const slotTime = centralSlotUTC(dateStr, h)
        ...
        if (isBlocked(w.counselor_id, dateStr, h)) continue
